...
class DataPreprocessor:
    def __init__(self):
        self.robust_scaler = RobustScaler(quantile_range=(25, 75))  # Используем RobustScaler для устойчивости к выбросам
        self.eps = 1e-8
        
    def fit_transform(self, data):
        # Применяем log1p трансформацию для уменьшения влияния выбросов
        data_log = np.log1p(np.abs(data) + self.eps)
        
        # Применяем RobustScaler
        data_transformed = self.robust_scaler.fit_transform(data_log.reshape(-1, 1)).reshape(-1)
        
        return data_transformed
    
    def transform(self, data):
        data_log = np.log1p(np.abs(data) + self.eps)
        return self.robust_scaler.transform(data_log.reshape(-1, 1)).reshape(-1)
    
    def inverse_transform(self, data):
        data_log = self.robust_scaler.inverse_transform(data.reshape(-1, 1)).reshape(-1)
        return np.exp(data_log) - 1 - self.eps

class DSPModel(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, emb_dim=128, output_dim=1):
        super(DSPModel, self).__init__()
...
def plot_training_history(train_losses, val_losses, save_path='outputs/dsp_implementation/plots'):
...
def plot_predictions_vs_targets(y_true, y_pred, save_path='outputs/dsp_implementation/plots'):
...
def plot_error_distribution(y_true, y_pred, save_path='outputs/dsp_implementation/plots'):
...
def plot_metrics_by_range(y_true, y_pred, save_path='outputs/dsp_implementation/plots'):
...
def plot_error_matrix(y_true, y_pred, save_path='outputs/dsp_implementation/plots'):
    ...
    axes[0,0].set_xlabel('Actual Values (DSP)')
    axes[0,0].set_ylabel('Predicted Values (DSP)')
    axes[0,0].grid(True)
    ...
    Mean Abs Error: {np.mean(abs_errors):.2f} DSP
    Median Abs Error: {np.median(abs_errors):.2f} DSP
    ...
    90th percentile Abs Error: {np.percentile(abs_errors, 90):.2f} DSP
    95th percentile Abs Error: {np.percentile(abs_errors, 95):.2f} DSP
    """
    ...
    error_stats = pd.DataFrame({
        'Metric': [
            'Mean Absolute Error (DSP)', 
            'Median Absolute Error (DSP)',
            '90th percentile Absolute Error (DSP)',
            '95th percentile Absolute Error (DSP)',
            'Mean Relative Error (%)',
...
def plot_confusion_matrix(y_true, y_pred, save_path='outputs/dsp_implementation/plots'):
    """Создание confusion matrix для диапазонов DSP"""
    ...
    plt.title('Confusion Matrix (DSP Ranges)')
...
def generate_all_plots(train_losses, val_losses, y_true, y_pred):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('outputs/dsp_implementation/plots/training_history.png')
    plt.close()
    ...
    plt.plot([y_true.min(), y_true.max()], [y_true.min(), y_true.max()], 'r--')
    plt.title('Predictions vs True Values')
    plt.xlabel('True Values (DSP)')
    plt.ylabel('Predictions (DSP)')
    plt.grid(True)
    r2 = r2_score(y_true, y_pred)
    plt.text(0.05, 0.95, f'R² = {r2:.4f}', transform=plt.gca().transAxes)
    plt.savefig('outputs/dsp_implementation/plots/predictions_vs_targets.png')
    plt.close()
    ...
    sns.histplot(errors, kde=True)
    plt.title('Error Distribution')
    plt.xlabel('Prediction Error (DSP)')
    plt.ylabel('Count')
    plt.grid(True)
    mean_error = np.mean(errors)
    std_error = np.std(errors)
    plt.text(0.05, 0.95, f'Mean Error: {mean_error:.4f}\nStd Error: {std_error:.4f}', 
             transform=plt.gca().transAxes)
    plt.savefig('outputs/dsp_implementation/plots/error_distribution.png')
    plt.close()
...
def main(args):
    ...
    graph_labels_dsp = pd.read_csv('outputs/processed_data/graph_target_dsp.csv')
    
    # Проверяем данные на NaN
    print("Checking input data...")
    print(f"Number of graphs: {len(graphs)}")
    print(f"Number of labels: {len(graph_labels_dsp)}")
    print(f"Labels contain NaN: {graph_labels_dsp.isna().any().any()}")
    
    # Нормализация целевой переменной
    scaler = StandardScaler()
    graph_labels_dsp_scaled = scaler.fit_transform(graph_labels_dsp.values)
    
    # Проверяем нормализованные данные
    print("Checking normalized data...")
    print(f"Contains NaN: {np.isnan(graph_labels_dsp_scaled).any()}")
    print(f"Min value: {np.min(graph_labels_dsp_scaled)}")
    print(f"Max value: {np.max(graph_labels_dsp_scaled)}")
    ...
    train_dataset = GraphDataset(
        ...
        graph_labels_dsp_scaled[train_idx]
    )
    val_dataset = GraphDataset(
        [graphs[i] for i in val_idx],
        graph_labels_dsp_scaled[val_idx]
    )
    test_dataset = GraphDataset(
        [graphs[i] for i in test_idx],
        graph_labels_dsp_scaled[test_idx]
    )
    ...
    # Создаем модель
    model = DSPModel(num_features, hidden_dim=128, emb_dim=128, output_dim=1).to(device)
    ...
    os.makedirs('outputs/dsp_implementation/metrics', exist_ok=True)
    
    # Сохраняем метрики
    with open('outputs/dsp_implementation/metrics/test_metrics_dsp.json', 'w') as f:
        json.dump(metrics, f, indent=4)
    
    # Сохраняем модель и данные
    os.makedirs('outputs/dsp_implementation/models', exist_ok=True)
    torch.save({
    ...
    }, 'outputs/dsp_implementation/models/dsp_model.pt')

    # Сохраняем полную модель для Netron
    torch.save(model, 'outputs/dsp_implementation/models/dsp_model_full.pt')
    ...
    def get_dsp_embeddings(model, loader, device):
        model.eval()
        embeddings = []
        with torch.no_grad():
    ...
    os.makedirs('outputs/dsp_implementation/embeddings', exist_ok=True)
    test_embeddings = get_dsp_embeddings(model, test_loader, device)
    np.save('outputs/dsp_implementation/embeddings/test_embeddings.npy', test_embeddings)

    return metrics

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='provide arguments for the graph embedding model with DSP predictions'
    )
